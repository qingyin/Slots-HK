local ProductGemsCell = class("ProductGemsCell", function()
...
function ProductGemsCell:ctor(prodInfo)

    self.schEntryArray = {}
    self.scheduler = require(cc.PACKAGE_NAME .. ".scheduler")
    self.rootNode  = CCBuilderReaderLoad(RES_CCBI.cashin_gems,self)
    ...
    self.totalLabel:setString(self.prodInfo.totalGamePoint)
    self.baseCoinLabel:setString(self.prodInfo.gamePoint)
...
function ProductGemsCell:setTimer()
    ...
    --self.timerLabel:setString(SHMG.formatValidTimeToStr(validTime))
...
function ProductGemsCell:updateAdTimeOut()
...
function ProductGemsCell:onSelected(selected)
...
function ProductGemsCell:onEnter()
end

function ProductGemsCell:onExit()
